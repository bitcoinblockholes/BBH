// Copyright (c) 2009-2012 bitcoinblockholes Developers
...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <bitcoinblockholesprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <bitcoinblockholesaddress>\n"
            "Reveals the private key corresponding to <bitcoinblockholesaddress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by bitcoinblockholes %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
